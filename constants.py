...
VIEWS = [SERVER_VIEW, SERVER_VIEW, SERVER_VIEW, SERVER_VIEW,
         SERVER_VIEW, SERVER_VIEW, SERVER_VIEW, SERVER_VIEW,
         SERVER_VIEW, SERVER_VIEW, SERVER_VIEW, SERVER_VIEW,
         SERVER_VIEW, SERVER_VIEW, SERVER_VIEW, SERVER_VIEW,
         SERVER_VIEW, SERVER_VIEW, SG_VIEW, CBLITE_DOTNET_VIEW,
         CBLITE_JAVA_VIEW, CBLITE_ANDROID_VIEW, CBLITE_IOS_VIEW]

FEATURES = {'server': SERVER_FEATURES, 'cblite': LITE_FEATURES,
            'sync_gateway': SG_FEATURES}
...
def random_build_query(collection):
    return "SELECT * FROM `filtered`.`collector_data`.`%s` where " \
           "`build`='7.0.0-%s'" % (collection, random.randint(3000,5000))

def random_component_query(collection):
    features = FEATURES[collection]
    return "SELECT `build` FROM `filtered`.`collector_data`.`%s` " \
           "where component='%s'" % (collection, random.choice(features).split('-')[1])

QUERIES = [
    "SELECT DISTINCT SPLIT(`build`,'-')[0] AS version FROM "
    "`filtered`.`collector_data`.`server` where SPLIT(`build`,'-')[0] is "
    "not missing ORDER BY version",
    "SELECT DISTINCT SPLIT(`build`,'-')[0] AS version FROM "
    "`filtered`.`collector_data`.`cblite` where SPLIT(`build`,'-')[0] "
    "is not missing ORDER BY version",
    "SELECT DISTINCT SPLIT(`build`,'-')[0] AS version FROM "
    "`filtered`.`collector_data`.`sync_gateway` where SPLIT(`build`,"
    "'-')[0] is not missing ORDER BY version",
    random_build_query('server'),
    random_build_query('cblite'),
    random_build_query('sync_gateway'),
    random_component_query('server'),
    random_component_query('cblite'),
    random_component_query('sync_gateway')
    ]